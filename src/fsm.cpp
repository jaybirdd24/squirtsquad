...
void FSM::approachLight()
{
    ...
    } else if (!avoidingObstacle && vx < Config::FLC_MIN_HOMING_VX) {
...
void FSM::sendTelemetry() {
    ...
    radio.sendLightData(
        motors.getHeading(),
        adcToVolts(lightCloseRightRaw),
        adcToVolts(lightLongRightRaw),
        adcToVolts(lightLongLeftRaw),
        adcToVolts(lightCloseLeftRaw)
    );
}
